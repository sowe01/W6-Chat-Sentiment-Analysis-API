...
from BaseDatos import creauser, user_coll, checkName, getUserSentiment, getAllSentiment
...
def crearchat(chat):
    ...
    new_id = user_coll.distinct("idUser")[-1] + 1
    new_idm = user_coll.distinct("idMessage")[-1] + 1
    new_idc = user_coll.distinct("idChat")[-1] + 1
    new_user = {
        "idUser": new_id,
        "userName": name,
        "idMessage": new_idm,
...
@app.route("/messages")
def getChat():
    return dumps(user_coll.find({}, {"text": 1, '_id': 0}))

# ver todas las conversaciones
@app.route("/chatid")
def getConversations():
    return dumps(user_coll.find({}, {'idChat': 1, 'text': 1, '_id': 0}))
